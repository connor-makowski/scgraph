...
def simplify_geojson(filename_in, filename_out:str|None=None, precision:int=4, pct_to_keep:int|float=100, min_points=3, silent:bool=False):
    ...
    for geometry in geometries:
        ...
        for line in coordinates:
            ...
            if idx % 10000 == 0:
                print_console(f"Simplified {idx} line geometries", end='\r', silent=silent)
...
def parse_geojson(filename_in, filename_out:str|None=None, precision:int=4, pct_to_keep:int|float=100, min_points=3, silent:bool=False):
    """
    ...
    """
    single_multi_line = simplify_geojson(filename_in, precision, pct_to_keep, min_points, silent)
    ...
    for idx, line in enumerate(single_multi_line):
        ...
        if idx % 10000 == 0:
            print_console(f"Processed {idx} of {total_geometries} geometries", end="\r", silent=silent)
    print_console(f"Processed {total_geometries} of {total_geometries} geometries", silent=silent)
    ...
    for idx, (key, value) in enumerate(odd_dict.items()):
        ...
        for end_node in value:
            graph_sub[node_map[end_node]] = hard_round(3, haversine(origin=start_node, destination=end_node))
        graph.append(graph_sub)
        if idx % 1000 == 0:
            print_console(f"Processed {idx} of {len(odd_dict)} nodes", end="\r", silent=silent)
    nodes = [[i[1], i[0]] for i in nodes]
    print_console(f"Processed {len(odd_dict)} of {len(odd_dict)} nodes", silent=silent)
