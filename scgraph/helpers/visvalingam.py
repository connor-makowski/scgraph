...
def triangle_area(p1: tuple[int|float, int|float], p2: tuple[int|float, int|float], p3: tuple[int|float, int|float]) -> int|float:
    """
...
def min_edge(p1: tuple[int|float, int|float], p2: tuple[int|float, int|float], p3: tuple[int|float, int|float]) -> int|float:
    """
...
def triangle_area_plus_min_edge(p1: tuple[int|float, int|float], p2: tuple[int|float, int|float], p3: tuple[int|float, int|float]) -> int|float:
    """
    Calculate the 2x the area of the triangle formed by points p1, p2, and p3,
...
def visvalingam(coords: list[list[int|float]], pct_to_keep: float, min_points: int = 2, weight_fn: callable = triangle_area) -> list[list[int|float]]:
    """
    ...
    nodes_to_keep = max(
        int(round((len(coords)-2) * pct_to_keep)) + 2,
        min_points
    )
    ...
    areas = [0]*len(coords)  # Areas of triangles formed by each point and its neighbors
    area_heap = []
    ...
    for idx in range(1, last_idx):
        area = triangle_area(coords[prevs[idx]], coords[idx], coords[nexts[idx]])
        areas[idx] = area
