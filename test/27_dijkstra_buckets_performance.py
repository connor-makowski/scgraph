...
def run_performance_test(name, graph_data, origin_id, destination_id):
    ...
    if py_buckets_time < py_dijkstra_time:
        print(f"SUCCESS (PY): Buckets was {py_dijkstra_time / py_buckets_time:.2f}x faster")
    else:
        print(f"INFO (PY): Buckets was {py_buckets_time / py_dijkstra_time:.2f}x slower")
    ...
    if HAS_CPP:
        ...
        start_time = time.time()
        cpp_buckets_result = cpp_graph.dijkstra_buckets(origin_id, destination_id)
        ...
        if cpp_buckets_time < cpp_dijkstra_time:
            print(f"SUCCESS (CPP): Buckets was {cpp_dijkstra_time / cpp_buckets_time:.2f}x faster")
        else:
            print(f"INFO (CPP): Buckets was {cpp_buckets_time / cpp_dijkstra_time:.2f}x slower")

        print(f"Overall C++ Buckets Speedup over Python Buckets: {py_buckets_time / cpp_buckets_time:.2f}x")
...
run_performance_test("200x200 Grid (Int weights 1-10)", graph_data, 0, size * size - 1)
...
run_performance_test("200x200 Grid (Float weights 0.1-10.0)", graph_data_float, 0, size * size - 1)
...
for name, rel_path in geograph_files:
    if os.path.exists(rel_path):
        ...
        run_performance_test(f"GeoGraph: {name} (Original Floats)", geograph.graph, 0, len(geograph.graph) // 2)
    else:
