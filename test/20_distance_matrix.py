...
def distance_matrix_time(nodes):
    ...
    print(
        f"Distance matrix ({len(nodes)} nodes, {len(nodes)**2} distances): {time_ms}ms total, {time_per_calc_us}us/distance"
    )
...
def naive_distance_matrix_time(nodes):
    ...
    for i in nodes:
        ...
        for j in nodes:
            out_sub.append(
                us_freeway_geograph.get_shortest_path(i, j)["length"]
            )
        out.append(out_sub)
    time_taken = time() - start
    time_per_calc = time_taken / (len(nodes) * len(nodes))
    ...
    time_per_calc_us = round(time_per_calc * 1e6, 4)
    print(
        f"Naive distance matrix ({len(nodes)} nodes, {len(nodes)**2} distances): {time_ms}ms total, {time_per_calc_us}us/distance"
    )
...
def haversine_time(nodes):
    ...
    time_per_calc_us = round(time_per_calc * 1e6, 4)
    print(
        f"Haversine distance ({len(nodes)} nodes, {len(nodes)*len(nodes)} distances): {time_ms}ms total, {time_per_calc_us}us/distance"
    )
