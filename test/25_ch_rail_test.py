...
def test_ch_rail():
    print("Testing CHGraph with North America Rail Geograph...")

    # Extract the underlying graph data correctly
    graph_data = north_america_rail_geograph.graph_object.graph
    ...
    for i in range(num_tests):
        ...
        try:
            ...
            if d_len == ch_len:
                success_count += 1
                print(
                    f"Test {i+1}: PASS ({origin} -> {dest}, len={d_len}, speedup={d_time/ch_time:.1f}x)"
                )
            else:
                print(
