    ...
    for city2, coord2 in cities.items():
        ...
        if city1 != city2 and city2 not in cities_fully_visited:
            length = us_freeway_geograph.get_shortest_path(
                ...
                destination_node={"longitude": coord2[1], "latitude": coord2[0]},
            )['length']
            ...
            if abs(length - cached_length) > 0.001:
                print(f"Length mismatch between uncached and cached for {city1} to {city2}: {length} vs {cached_length}")
                success = False
...
time_test(
    name="GeoGraph cached time",
    thunk=cached_time
)

time_test(
    name="GeoGraph uncached time",
    thunk=uncached_time
)
